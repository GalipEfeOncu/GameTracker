...
export default function RegisterPage() {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            const message = err.response?.data?.message ?? err.response?.data ?? err.message ?? 'Kayıt sırasında bir hata oluştu.';
    ...
    const handleVerify = async (e) => {
        ...
        try {
            ...
            const message = err.response?.data?.message ?? err.response?.data ?? err.message ?? 'Kod geçersiz veya süresi dolmuş olabilir.';
            setError(typeof message === 'string' ? message : 'Doğrulama başarısız.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="flex h-screen w-full items-center justify-center bg-[#0f111a]">
            <div className={cardClass}>
                <div className="mb-8 flex flex-col items-center">
                    ...
                    <h1 className="text-xl font-semibold tracking-tight text-white">Hesap Oluştur</h1>
                    <p className="mt-1.5 text-center text-sm text-gray-500">GameTracker dünyasına katılın ve kütüphanenizi yönetin.</p>
                </div>
                ...
                {step === 'success' ? (
                    <div className="flex flex-col items-center py-10">
                        ...
                        <h2 className="text-lg font-semibold text-white">E-posta doğrulandı</h2>
                        <p className="mt-1 text-center text-sm text-gray-500">Hesabınız hazır. Giriş sayfasına yönlendiriliyorsunuz...</p>
                    </div>
                ) : step === 'verify' ? (
                    <form onSubmit={handleVerify} className="space-y-4">
                        <div className="flex items-center gap-3 border border-[#1f2334] bg-[#1a1e2d] px-3 py-2.5 text-sm text-gray-400">
                            <Mail size={18} />
                            <span>E-postanıza gönderilen 6 haneli doğrulama kodunu girin.</span>
                        </div>
                        {error && (
                        ...
                        <button type="submit" disabled={isLoading || verificationCode.length !== 6} className={`mt-4 ${btnClass}`}>
                            {isLoading ? <Loader2 size={18} className="animate-spin" /> : 'Doğrula ve giriş yap'}
                        </button>
                        <p className="mt-6 text-center text-sm text-gray-500">
                            Zaten hesabınız var mı? <Link to="/login" className={linkClass}>Giriş Yap</Link>
                        </p>
                    </form>
                ) : (
                    <form onSubmit={handleRegister} className="space-y-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className={inputClass} placeholder="Örn: gamer123" required />
                        </div>
                        <div className="space-y-1">
                            <label className={labelClass}>E-posta</label>
                            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className={inputClass} placeholder="eposta@adresiniz.com" required />
                        </div>
                        <div className="space-y-1">
                        ...
                        <button type="submit" disabled={isLoading} className={`mt-4 ${btnClass}`}>
                            {isLoading ? <Loader2 size={18} className="animate-spin" /> : 'Kayıt ol ve e-posta doğrulama kodu al'}
                        </button>
